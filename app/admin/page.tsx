  ...
  return (
    <div className="admin-shell">
      <aside className={open ? "admin-sidebar open" : "admin-sidebar"}>
        ...
        <nav>{nav.map(([label, Icon], i) => <a className={i === 0 ? "active" : ""} href={label === "Content Studio" ? "/admin/content" : label === "Media Library" ? "/admin/media" : label === "Pages" ? "/admin/pages" : "#"} key={label}><Icon size={17}/>{label}</a>)}</nav>
