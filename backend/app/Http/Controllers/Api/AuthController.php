...
class AuthController extends Controller {
 ...
 public function verifyEmail(Request $request,int $id,string $hash){$u=$request->user();abort_unless($u->id===$id,403);abort_unless(hash_equals(sha1($u->getEmailForVerification()),$hash),403);if(!$u->hasVerifiedEmail()){$u->markEmailAsVerified();}return response()->json(['message'=>'Email verified.']);}
